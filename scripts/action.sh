...
if is_process_running "mihomo"; then
    ...
    ${module_dir}/scripts/clash.sh disable && ${module_dir}/scripts/dns.sh enable ${redir_port}
else
    echo "正在启动akashaProxy."
    if [ ${MIHOMO_DNS_ENABLE} = true ] ; then
        ${module_dir}/scripts/clash.sh enable  && ${module_dir}/scripts/dns.sh enable ${MIHOMO_DNS_PORT}
    else
        ${module_dir}/scripts/clash.sh enable  && ${module_dir}/scripts/dns.sh enable ${redir_port}
    fi
fi
